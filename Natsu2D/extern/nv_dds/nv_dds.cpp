...
bool CDDSImage::load(natStream* pStream, bool flipImage, long pos)
{
	DDS_HEADER ddsh;
	char filecode[4];
	//FILE *fp;
	...
	// read in file marker, make sure its a DDS file
	//fread(filecode, 1, 4, fp);
	pStream->ReadBytes(reinterpret_cast<nData>(filecode), 4);
	if (strncmp(filecode, "DDS ", 4) != 0)
	{
		//fclose(fp);
		return false;
	...
	// read in DDS header
	//fread(&ddsh, sizeof(ddsh), 1, fp);
	pStream->ReadBytes(reinterpret_cast<nData>(&ddsh), sizeof(ddsh));
	...
	for (int n = 0; n < (cubemap ? 6 : 1); n++)
	{
		...
		CTexture img(width, height, depth, size);
		//fread(img, 1, img.size, fp);
		pStream->ReadBytes(reinterpret_cast<nData>(img.operator char *()), img.size);
		...
		for (int i = 0; i < numMipmaps && (w || h); i++)
		{
			...
			CSurface mipmap(w, h, d, size);
			//fread(mipmap, 1, mipmap.size, fp);
			pStream->ReadBytes(reinterpret_cast<nData>(mipmap.operator char *()), mipmap.size);
